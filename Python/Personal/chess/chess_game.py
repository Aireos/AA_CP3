...
#class that makes a enum for white and black colors
class Color(Enum):
    white = 1
    black = 2

#defines basic class for all chess pieces
...
class King(ChessPiece):
    def can_move(self, new_position, board):
        ...
        board.board[original_position] = None
        self.position = new_position
        board.board[new_position] = self

        in_check = board.is_king_in_check(self.color)

        self.position = original_position
        board.board[original_position] = self
        board.board[new_position] = target_piece
...
#class that defines all functions neccacary for game run
class ChessGame:
    def __init__(self):
